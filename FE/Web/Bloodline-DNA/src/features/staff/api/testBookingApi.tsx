...
export const getTestBookingApi = async (token: string): Promise<TestBookingResponse[]> => {
  ...
  while (attempts < maxRetries) {
    try {
      const response = await rootApi.get<{ data: TestBookingResponse[] }>("/TestBooking", {
        headers: {
      ...
      if (attempts >= maxRetries) {
        throw new Error(
          error instanceof Error
            ? `Failed to get test bookings: ${error.message}`
            : "Failed to get test bookings: Unknown error"
        );
      }
