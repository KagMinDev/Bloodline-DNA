  ...
  try {
    ...
    if (!response.ok) {
        ...
        if (errorData.message) {
          ...
          if (errorData.innerException || errorData.details) {
            detailedMessage += ` Details: ${
              errorData.innerException || errorData.details
            }`;
          }
        }
      ...
      if (response.status === 401) {
        throw new Error(
          `Unauthorized: Please login to continue. Status: ${response.status}`
        );
      }
...
const getTestServiceIdMapping = (
  ...
  if (selectedService?.testServiceInfo?.id) {
    console.log(
      "Using testServiceInfo.id as testServiceId:",
  ...
  if (selectedService?.testServiceInfo) {
    ...
    const testServiceId =
      testServiceInfo.testServiceId ||
      testServiceInfo.serviceId ||
      testServiceInfo.id;
    if (testServiceId) {
      console.log(
        "Using testServiceInfo field as testServiceId:",
        testServiceId
      );
  ...
  if (selectedService?.id && selectedService.id !== "temp-id") {
    console.log(
      "Fallback: Using selectedService.id as testServiceId:",
      selectedService.id
    );
  ...
  if (category === "civil") {
    ...
    if (serviceType === "home") {
      return (
        selectedService?.testServiceInfo?.id ||
        selectedService?.id ||
        "legal-home-test-service-id"
      ); // Replace with real ID
    } else if (serviceType === "clinic") {
      return (
        selectedService?.testServiceInfo?.id ||
        selectedService?.id ||
        "legal-clinic-test-service-id"
      ); // Replace with real ID
    }
  }
...
export const mapFormDataToBookingRequest = async (
  ...
  let testServiceId = getTestServiceIdMapping(
    selectedService,
  ...
  if (!testServiceId) {
    ...
    throw new Error(
      "Unable to determine TestService ID. Please select a valid service."
    );
  }
  ...
  if (!guidRegex.test(testServiceId)) {
    ...
    throw new Error(
      "Invalid service ID format. Please try selecting the service again."
    );
  }
  ...
  if (
    !realClientId ||
    realClientId === "default-client-id" ||
    realClientId === "temp-client-id"
  ) {
  ...
  const priceServiceId =
    selectedService?.id || selectedService?.serviceId || testServiceId;
  ...
  if (realClientId) {
    ...
    throw new Error(
      "Unable to get user ID. Please make sure you are logged in."
    );
  }
  ...
  if (
    !bookingRequest.testServiceId ||
    bookingRequest.testServiceId.length < 10
  ) {
    console.error(
      "❌ Invalid testServiceId in final request:",
      bookingRequest.testServiceId
    );
  ...
  if (!bookingRequest.clientName || bookingRequest.clientName.length < 2) {
    console.error(
      "❌ Invalid clientName in final request:",
  ...
  if (
    !bookingRequest.priceServiceId ||
    bookingRequest.priceServiceId.length < 10
  ) {
    console.error(
      "❌ Invalid priceServiceId in final request:",
      bookingRequest.priceServiceId
    );
  ...
  if (
    ...
    !bookingRequest.appointmentDate.includes("T")
  ) {
    console.error(
      "❌ Invalid appointmentDate in final request:",
      bookingRequest.appointmentDate
    );
